...
class AdzunaJobService:
    ...
    def search_jobs(self, 
        ...
        try:
            ...
            # Handle remote job searches - India API doesn't recognize "Remote" as location
            original_what = what
            original_where = where
            is_remote_search = where.lower() in ['remote', 'work from home', 'wfh']
            
            if is_remote_search:
                print("🏠 Detected remote job search - optimizing for India market...")
                # Strategy: Search all locations first, then filter for remote in results
                where = ""  # Search all locations
                print(f"📝 Modified search: '{what}' in '{where}' (will include remote-friendly results)")
            ...
            if response.status_code == 200:
                ...
                # If remote search returned 0 results, try fallback strategies
                if is_remote_search and len(jobs.get('results', [])) == 0:
                    print("🔄 Remote search returned 0 results, trying fallback strategies...")
                    
                    # Fallback 1: Search with "remote" in job title
                    print("📝 Fallback 1: Adding 'remote' to job title...")
                    fallback_jobs = self._search_with_remote_keywords(original_what, results_per_page)
                    if len(fallback_jobs) > 0:
                        jobs['results'] = fallback_jobs
                        jobs['count'] = len(fallback_jobs)
                        print(f"✅ Fallback successful! Found {len(fallback_jobs)} jobs with remote keywords")
                    else:
                        # Fallback 2: Search major Indian cities
                        print("📝 Fallback 2: Searching major Indian cities...")
                        city_jobs = self._search_major_cities(original_what, results_per_page)
                        if len(city_jobs) > 0:
                            jobs['results'] = city_jobs
                            jobs['count'] = len(city_jobs)
                            print(f"✅ City search successful! Found {len(city_jobs)} jobs in major cities")
                ...
                return {
                    ...
                    'current_page': page,
                    'search_params': {
                        'what': original_what,  # Return original search terms
                        'where': original_where,
                        'sort_by': sort_by
                    },
                    'actual_search': {  # Show what was actually searched
                        'what': what,
                        'where': where
                    },
                    'timestamp': time.time()
    ...
    def _search_with_remote_keywords(self, job_title: str, results_per_page: int) -> List[Dict]:
        """Helper method to search with remote keywords in job title"""
        try:
            # Try different remote keyword combinations
            remote_variations = [
                f"{job_title} remote",
                f"remote {job_title}",
                f"{job_title} work from home",
                f"{job_title} wfh"
            ]
            
            all_jobs = []
            for variation in remote_variations:
                endpoint = f"{self.base_url}/jobs/{self.country}/search/1"
                params = {
                    'app_id': self.app_id,
                    'app_key': self.app_key,
                    'results_per_page': min(results_per_page // len(remote_variations), 10),
                    'what': variation,
                    'where': '',
                    'sort_by': 'relevance'
                }
            ...
            print(f"⚠️ Error in remote keyword search: {e}")
            return []

    def _search_major_cities(self, job_title: str, results_per_page: int) -> List[Dict]:
        """Helper method to search major Indian cities for jobs"""
        try:
            major_cities = ['bangalore', 'mumbai', 'delhi', 'pune', 'hyderabad', 'chennai']
            all_jobs = []
            jobs_per_city = max(1, results_per_page // len(major_cities))
            ...
            for city in major_cities:
                ...
                params = {
                    'app_id': self.app_id,
                    'app_key': self.app_key,
                    'results_per_page': jobs_per_city,
                    'what': job_title,
                    'where': city,
                    'sort_by': 'relevance'
                }
                ...
                if response.status_code == 200:
                    data = response.json()
                    jobs = self.process_job_results(data)
                    city_jobs = jobs.get('results', [])
                    
                    # Add city info to job listings
            ...
            print(f"⚠️ Error in city search: {e}")
            return []
